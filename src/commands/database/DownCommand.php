...
class DownCommand extends AbstractEnvCommand
{
    protected function configure()
    {
        ...
        $this->setDescription('Rollback all waiting migration down to [to] option if precised');

        $this->addArgument('env', InputArgument::REQUIRED, 'Environment');

        $this->addOption('to', null, InputOption::VALUE_REQUIRED, 'Migration will be downed to this migration id included.');
        $this->addOption('only', null, InputOption::VALUE_REQUIRED, 'If you need to down this migration id only.');
